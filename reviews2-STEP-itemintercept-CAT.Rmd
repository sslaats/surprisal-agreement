---
title: "reviews2_surprisal-agreement-intercept-CATEGORICAL"
author: "S. Slaats"
date: "2025-02-25"
...
```{r word index 6 viz permutation, fig.width=10,fig.height=4, include=TRUE, message=FALSE, warning=FALSE}
...
# we remove the random factor with the smallest contribution (agreement * number)
# also does not work
# we remove the random intercept per participant; also does not converge
# remove the interaction between surprisal and agreement; also does not converge
# then we remove random effect of number; finally, it converges
mdl7 <- lmer('logRTresidual ~ agreement + surprisal + (agreement + surprisal | UserId) + (1 | set_number)',
             subset(data, word_index==7), 
             control=lmerControl(optimizer="nloptwrap"), 
             REML=FALSE)
...
writexl::write_xlsx(list("target_random" = cbind(rownames(as.data.frame(comparison5[1])), as.data.frame(comparison5[1])),
                ...
                'C:/Users/slaats/Documents/MPI/Project/Project 3 - Behavioural studies/3 - SPR main/analysis/stats_modelcomp_categorical.xlsx')
```