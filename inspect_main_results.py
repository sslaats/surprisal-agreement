...
RTs = pd.read_csv('K:/Project/Project 3 - Behavioural studies/3 - SPR main/analysis/readingtimes_88pp.csv')
...
# take condition mean per participant, remove all that are 2.5 sds away from that
for pp in RTs['UserId'].unique():
    
    pp_dat = RTs.loc[RTs['UserId'] == pp]
    
    for agr in ['correct','incorrect']:
        for surp in ['low', 'high']:

            # sd & mean
            cond_sd = np.std(pp_dat.loc[(pp_dat['agreement'] == agr) & (pp_dat['surprisal'] == surp), 'logRT'])
            cond_mean = np.mean(pp_dat.loc[(pp_dat['agreement'] == agr) & (pp_dat['surprisal'] == surp), 'logRT'])
            
            # get maximum value
            max_value = cond_mean + 2.5 * cond_sd
            min_value = cond_mean - 2.5 * cond_sd
            
            # add the limits to the dataframe
            RTs.loc[(RTs['UserId'] == pp) & 
                    (RTs['agreement'] == agr) &  
                    (RTs['surprisal'] == surp), 'max_values'] = [max_value] * len(RTs.loc[(RTs['UserId'] == pp) & 
                                                                                             (RTs['agreement'] == agr) &  
                                                                                             (RTs['surprisal'] == surp)])
            RTs.loc[(RTs['UserId'] == pp) & 
                    (RTs['agreement'] == agr) &  
                    (RTs['surprisal'] == surp), 'min_values'] = [min_value] * len(RTs.loc[(RTs['UserId'] == pp) & 
                                                                                             (RTs['agreement'] == agr) &  
                                                                                             (RTs['surprisal'] == surp)])
...
print(f'Removed: {(data_length-len(RTs))/data_length}')
# in total, 2.8 % of the data was removed

#RTs.to_csv('K:/Project/Project 3 - Behavioural studies/3 - SPR main/analysis/readingtimes_88pp_preprocessed.csv')

# %% compute summed reading times
RTs_tosum = RTs.loc[RTs['word_index'].isin([5,6,7,8])]
summed_RT = RTs_tosum.groupby(by=['agreement', 'surprisal', 'list', 'sentence_order', 'set_number', 'UserId', 'surprisal_value']).aggregate(sumRT=pd.NamedAgg('RT', np.sum),
                                                                                                         sumWF=pd.NamedAgg('word_frequency', np.sum),
                                                                                                         sumWL=pd.NamedAgg('word_length', np.sum))
summed_RT['log_sumRT'] = np.log(summed_RT['sumRT'])
summed_RT.to_csv('K:/Project/Project 3 - Behavioural studies/3 - SPR main/analysis/readingtimes_88pp_summed.csv')
# regress against: word length (number of letters) and list position (sentence number)
