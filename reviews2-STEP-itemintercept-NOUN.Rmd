---
title: "reviews2_surprisal-agreement-intercept-NOUN"
author: "S. Slaats"
date: "2025-02-25"
...
fmla <- 'logRTresidual ~ agreement * surprisal_value + agreement * noun_number + 
                              (1 + agreement * surprisal_value + agreement * noun_number | UserId) + 
                              (1 | set_number )'
```
...
```{r word index 5 viz permutation, fig.width=5,fig.height=4, include=TRUE, message=FALSE, warning=FALSE}
...
mdl6_sur_by_agr_raw <- summary(pairs(mdl6_sur_by_agr), adjust='none')

# extract pvalues
all_pvals <- mdl6_sur_by_agr_raw$p.value
adj_pvals <- p.adjust(all_pvals, method='bonferroni')

# extract pvalues
mdl6_res_pair <- rbind(mdl6_sur_by_agr_raw) 
...
writexl::write_xlsx(list("target_random" = cbind(rownames(as.data.frame(comparison5[1])), as.data.frame(comparison5[1])),
                ...
                'C:/Users/slaats/Documents/MPI/Project/Project 3 - Behavioural studies/3 - SPR main/analysis/stats_modelcomp_noun.xlsx')
```